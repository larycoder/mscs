...
species people skills: [pedestrian, moving] parallel: true control: simple_bdi {
	...
	plan lets_wander intention: shopping finished_when: has_desire(found_all_product) or has_desire(loose_patience) or has_desire(need_pay) or has_desire(need_leave) {
		do moveAround;
	}
	...
	reflex everyday_calculation when: every(daily #cycle) and cycle > 10 {
		...
		if (length(boughtList) = 0) {
			...
			bought_per_shoppingList <- (1.0 - length(productList) / length(boughtList));
		}
...
experiment test type: gui {
	output {
		...
		display "Total status" {
			chart "Total status" type: histogram x_label: '' y_label: '' {
		...
		display "Round status" refresh: every(daily) {
			chart "Past information" type: series x_label: '' y_label: '' {
